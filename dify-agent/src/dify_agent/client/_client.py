...
class Client:
    ...
    context manager. Owned HTTPX clients use ``trust_env=False`` so a developer
    machine HTTP proxy cannot turn a down Agent server into a misleading 502.
    """
    ...
    def _get_sync_http_client(self) -> httpx.Client:
        ...
        if self._sync_http_client is None:
            self._sync_http_client = httpx.Client(
                timeout=self._timeout, headers=self._headers, trust_env=False
            )
