...
def test_owned_http_clients_ignore_process_proxy_env() -> None:
    client = Client(base_url="http://127.0.0.1:5050")
    try:
        assert client._get_sync_http_client().trust_env is False
    finally:
        client.close_sync()
    async_client = Client(base_url="http://127.0.0.1:5050")

    async def _check_async() -> None:
        http_client = async_client._get_async_http_client()
