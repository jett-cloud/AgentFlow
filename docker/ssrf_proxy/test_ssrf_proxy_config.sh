...
# Local API uses CODE_EXECUTION_ENDPOINT=http://127.0.0.1:8194, so Host is 127.0.0.1.
# The reverse proxy must still forward to sandbox instead of looping on vhost.
assert_sandbox_bridge_ignores_loopback_host() {
  local target_url="$1"
  local output
  local status_code
  ...
  output="$(
    docker run \
      ...
      --network "$NETWORK_NAME" \
      "$CLIENT_IMAGE" \
      wget -S -O /dev/null -T 10 --header "Host: 127.0.0.1:8194" "$target_url" 2>&1 || true
  )"
  status_code="$(printf '%s\n' "$output" | awk '$1 ~ /^HTTP\// { code = $2 } END { print code }')"
  if [[ ! "$status_code" =~ ^2[0-9][0-9]$ ]]; then
    echo "Expected sandbox host bridge to ignore Host 127.0.0.1, got ${status_code:-no response}."
    docker logs "$CONTAINER_NAME" >&2 || true
    docker logs "$SANDBOX_CONTAINER_NAME" >&2 || true
    exit 1
  fi
...
assert_sandbox_bridge_ignores_loopback_host "http://$CONTAINER_NAME:8194/health"
