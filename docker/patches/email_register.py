"""Image-compatible Console auth register module with /register (+ /dev-register alias).

Based on langgenius/dify-api:1.16.0-rc1 email_register.py, plus a self-service
register endpoint used by agent-flow-frontend.
"""
...
from controllers.console.wraps import decrypt_password_field
...
class DevelopmentRegisterApi(Resource):
    ...
    def post(self):
        def _flag(name: str) -> bool:
            if bool(getattr(dify_config, name, False)):
                return True
            return os.getenv(name, "false").lower() in {"1", "true", "yes", "on"}

        if not (_flag("ALLOW_REGISTER") or _flag("ALLOW_DEV_NO_EMAIL_REGISTER")):
            abort(404)
