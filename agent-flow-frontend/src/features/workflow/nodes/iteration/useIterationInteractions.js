// src/views/copilot/components/workflow/node/iteration/useIterationInteractions.js
// 1:1 对齐 Dify React 官方源码 (web/app/components/workflow/nodes/iteration/use-interactions.helpers.ts)
...
/**
 * 1. 计算包含所有内部子节点的最大外包围盒 (Container Bounds)
 */
export const getIterationContainerBounds = (childrenNodes = []) => {
...
/**
 * 2. 算出现在父迭代容器所需要自动调整扩展的 Width 和 Height
 */
export const getIterationContainerResize = (currentNode, bounds) => {
  ...
  if (bounds.rightNode) {
    ...
    const requiredW = bounds.rightNode.position.x + rightWidth + ITERATION_PADDING.right
  ...
  if (bounds.bottomNode) {
    ...
    const requiredH = bounds.bottomNode.position.y + bottomHeight + ITERATION_PADDING.bottom
 ...
 * 限制坐标：无法拖出迭代框左边与顶边，亦无法超出父容器右边与底边
 */
export const getRestrictedIterationPosition = (node, parentNode) => {
  ...
  // 右下角边缘锁定
  if (node.position.x + nodeWidth > parentWidth - ITERATION_PADDING.right) {
    restrictPosition.x = parentWidth - ITERATION_PADDING.right - nodeWidth
  }
  if (node.position.y + nodeHeight > parentHeight - ITERATION_PADDING.bottom) {
    restrictPosition.y = parentHeight - ITERATION_PADDING.bottom - nodeHeight
  }
...
/**
 * 4. 获得迭代容器内部所有排除 start 节点之外的实际业务子节点
 */
export const getIterationChildren = (nodes = [], nodeId) => {
  return nodes.filter(
    (n) => n.parentNode === nodeId && n.type !== CUSTOM_ITERATION_START_NODE && n.type !== 'iteration-start'
  )
}
