...
class TestInitialization:
    ...
    def test_implementation_parses_optional_icons(self):
        server_info = Implementation.model_validate(
            {
                "name": "test-server",
                "version": "1.0.0",
                "icons": [{"src": "https://test-server.example/icon.png", "mimeType": "image/png"}],
            }
        )

        assert server_info.icons is not None
        assert server_info.icons[0].src == "https://test-server.example/icon.png"
