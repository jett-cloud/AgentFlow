...
def test_uploaded_workflow_files_are_included_without_prompt_marker():
    ...
    assert DIFY_INPUT_LAYER_ID not in _request_layers(result)


def test_uploaded_workflow_images_are_injected_as_vision_input_when_model_supports_vision(monkeypatch):
    image_reference = build_file_reference(record_id="uploaded-image-1")

    class UploadedImagesVariablePool(FakeVariablePool):
        def get(self, selector):
            if list(selector) == ["sys", "files"]:
                return ArrayFileSegment(
                    value=[
    ...
    monkeypatch.setattr(
        File,
        "generate_url",
        lambda self, for_external=True: "https://files.example/signed/swatch.png",
    )
    context = replace(_context(), variable_pool=UploadedImagesVariablePool())
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Identify the color and return the knowledge-base code.",
        }
    )

    result = WorkflowAgentRuntimeRequestBuilder(
        credentials_provider=FakeCredentialsProvider(),
        vision_support_checker=lambda *_args: True,
    ).build(context)

    layers = _request_layers(result)
    assert DIFY_INPUT_LAYER_ID in layers
    assert layers[DIFY_INPUT_LAYER_ID]["type"] == "dify.input"
    assert layers[DIFY_INPUT_LAYER_ID]["config"]["images"] == [
        {
    ...
    # Images remain discoverable via the text download mapping for sandbox tools.
    assert _uploaded_workflow_files_prompt_payload(result) == [
        {
            "transfer_method": "local_file",
            "reference": image_reference,
...
def test_previous_node_images_are_injected_as_vision_input_when_model_supports_vision(monkeypatch):
    cutout_reference = build_file_reference(record_id="cutout-image-1")

    class PreviousNodeImageVariablePool(FakeVariablePool):
        def get(self, selector):
            if list(selector) == ["agent1", "cutout_image"]:
                return FileSegment(
                    value=File(
                        type=FileType.IMAGE,
                        transfer_method=FileTransferMethod.TOOL_FILE,
                        reference=cutout_reference,
                        remote_url=None,
                        filename="cutout.png",
                        extension=".png",
                        mime_type="image/png",
    ...
    monkeypatch.setattr(
        File,
        "generate_url",
        lambda self, for_external=True: "https://files.example/signed/cutout.png",
    )
    context = replace(_context(), variable_pool=PreviousNodeImageVariablePool())
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Analyze colors in {{#agent1.cutout_image#}} and assemble a bead prompt.",
        }
    )
    ...
    assert layers[DIFY_INPUT_LAYER_ID]["config"]["images"] == [
        {
            "url": "https://files.example/signed/cutout.png",
            "mime_type": "image/png",
            "filename": "cutout.png",
            "detail": "high",
        }
    ]
    assert _previous_node_prompt_payload(result, "agent1.cutout_image") == {
        "transfer_method": "tool_file",
        "reference": cutout_reference,
    }


def test_previous_node_document_files_are_not_injected_as_vision_input():
    doc_reference = build_file_reference(record_id="upstream-doc-1")

    class PreviousNodeDocumentVariablePool(FakeVariablePool):
        def get(self, selector):
            if list(selector) == ["agent1", "report"]:
                return FileSegment(
                    value=File(
                        type=FileType.DOCUMENT,
                        transfer_method=FileTransferMethod.TOOL_FILE,
    ...
    context = replace(_context(), variable_pool=PreviousNodeDocumentVariablePool())
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Summarize {{#agent1.report#}}.",
        }
    )
    ...
    assert DIFY_INPUT_LAYER_ID not in _request_layers(result)
    assert _previous_node_prompt_payload(result, "agent1.report") == {
        "transfer_method": "tool_file",
        "reference": doc_reference,
    }


def test_duplicate_sys_and_previous_node_images_are_deduped_for_vision(monkeypatch):
    shared_reference = build_file_reference(record_id="shared-image-1")

    class SharedImageVariablePool(FakeVariablePool):
        def get(self, selector):
            image = File(
                type=FileType.IMAGE,
                transfer_method=FileTransferMethod.LOCAL_FILE,
            ...
            if list(selector) == ["sys", "files"]:
                return ArrayFileSegment(value=[image])
            if list(selector) == ["agent1", "cutout_image"]:
                return FileSegment(value=image)
            return super().get(selector)

    monkeypatch.setattr(
        File,
        "generate_url",
        lambda self, for_external=True: "https://files.example/signed/shared.png",
    )
    context = replace(_context(), variable_pool=SharedImageVariablePool())
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Use {{#agent1.cutout_image#}}.",
        }
    )
    ...
    images = _request_layers(result)[DIFY_INPUT_LAYER_ID]["config"]["images"]
    assert len(images) == 1
    assert images[0]["url"] == "https://files.example/signed/shared.png"


def test_non_image_uploaded_files_are_not_injected_as_vision_input():
    file_reference = build_file_reference(record_id="uploaded-doc-1")

    class UploadedDocumentVariablePool(FakeVariablePool):
        def get(self, selector):
            if list(selector) == ["sys", "files"]:
                return ArrayFileSegment(
                    value=[
    ...
    context = replace(_context(), variable_pool=UploadedDocumentVariablePool())
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Read the uploaded document.",
        }
    )
...
def test_uploaded_workflow_images_are_skipped_when_model_lacks_vision(monkeypatch):
    image_reference = build_file_reference(record_id="uploaded-image-2")
    ...
    context.binding.node_job_config = WorkflowNodeJobConfig.model_validate(
        {
            "workflow_prompt": "Describe the uploaded image.",
        }
    )

    result = WorkflowAgentRuntimeRequestBuilder(
        credentials_provider=FakeCredentialsProvider(),
        vision_support_checker=lambda *_args: False,
    ).build(context)
    ...
    assert any(item["code"] == "vision_not_supported" for item in warnings)
