...
def test_build_layers_refreshes_each_mcp_provider_snapshot_once() -> None:
    runtime_provider = FakeRuntimeProvider(_tool())
    refreshed: list[tuple[str, str]] = []

    def refresher(*, tenant_id: str, provider_id: str) -> None:
        refreshed.append((tenant_id, provider_id))

    builder = WorkflowAgentDifyToolsBuilder(
        tool_runtime_provider=runtime_provider,
        mcp_provider_id_resolver=lambda *, tenant_id, provider_id: "mcp-server-1",
        mcp_snapshot_refresher=refresher,
    )
    tools = AgentSoulToolsConfig.model_validate(
        {
            "dify_tools": [
    ...
    assert refreshed == [("tenant-1", "mcp-server-1")]


def test_build_layers_keeps_cached_mcp_tools_when_snapshot_refresh_fails() -> None:
    runtime_provider = FakeRuntimeProvider(_tool())
