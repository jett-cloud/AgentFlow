...
from core.workflow.runtime import workflow_entry
...
class TestWorkflowEntryTracing:
    def test_terminal_event_finishes_hooks_before_consumer_stops(self):
        event = NodeRunSucceededEvent(
            id="execution", node_id="node", node_type=BuiltinNodeTypes.END, start_at=datetime(2026, 1, 1)
        )
        layer = MagicMock()
        ...
        with patch.object(workflow_entry, "ObservabilityLayer", return_value=layer):
            events = workflow_entry.WorkflowEntry._traced_node_run(node)
            assert next(events) is event
            layer.on_node_run_end.assert_called_once_with(node, None, event)
            layer.on_graph_end.assert_called_once_with(None)
            events.close()

        layer.on_node_run_end.assert_called_once()
        layer.on_graph_end.assert_called_once()

    @pytest.mark.parametrize("node_type", [BuiltinNodeTypes.LOOP, BuiltinNodeTypes.ITERATION])
    def test_container_debug_requires_engine_endpoint(self, node_type):
        workflow = SimpleNamespace(
            get_node_config_by_id=lambda _: {"data": BaseNodeData(type=node_type)},
        )
    ...
    def test_traced_node_run_reports_success(self):
        ...
        layer.initialize.assert_called_once()
        layer.on_graph_end.assert_called_once_with(None)
