...
def _invoke_remote_raising(error: Exception) -> None:
    tool = _build_mcp_tool()
    provider = MagicMock()
    provider.decrypt_server_url.return_value = "https://mcp.example.com"
    provider.decrypt_headers.return_value = {}
    provider.retrieve_tokens.return_value = None
    service = MagicMock()
    service.get_provider_entity.return_value = provider
    with (
        patch("extensions.ext_database.db") as mock_db,
        patch("sqlalchemy.orm.Session") as session_cls,
        patch("services.tools.mcp_tools_manage_service.MCPToolManageService", return_value=service),
        patch("core.tools.mcp_tool.tool.MCPClientWithAuthRetry", side_effect=error),
        patch("core.tools.mcp_tool.tool.dify_config") as cfg,
    ):
        mock_db.engine = MagicMock()
        cfg.ENTERPRISE_ENABLED = False
        session_cls.return_value.__enter__.return_value = MagicMock()
        tool.invoke_remote_mcp_tool({"q": "1"}, user_id="user-1")


def test_invoke_remote_maps_auth_error_to_credential_validation():
    with pytest.raises(ToolProviderCredentialValidationError, match="MCP authentication failed"):
        _invoke_remote_raising(MCPAuthError("401"))


def test_invoke_remote_maps_connection_error_to_tool_invoke_error():
    with pytest.raises(ToolInvokeError, match="Failed to connect to MCP server") as exc_info:
        _invoke_remote_raising(MCPConnectionError("timeout"))
