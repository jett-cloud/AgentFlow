...
from dify_agent.layers.input import DIFY_INPUT_LAYER_TYPE_ID, DifyInputImageConfig, DifyInputLayerConfig
...
def test_workflow_request_builder_omits_config_layer_when_shell_disabled():
    run_input = _run_input()
    run_input.include_shell = False
    run_input.config_layer_config = DifyConfigLayerConfig(agent_id="agent-1")

    request = AgentBackendRunRequestBuilder().build_for_workflow_node(run_input)
    names = {layer.name for layer in request.composition.layers}

    assert DIFY_SHELL_LAYER_ID not in names
    assert DIFY_CONFIG_LAYER_ID not in names
...
def test_request_builder_injects_vision_input_layer_after_user_prompt():
    run_input = _run_input().model_copy(
        update={
            "vision_images": [
                DifyInputImageConfig(
                    url="https://files.example/signed/color-card.png",
                    mime_type="image/png",
                    filename="color-card.png",
                    detail="high",
                )
            ]
        }
    ...
    assert layers[DIFY_INPUT_LAYER_ID].type == DIFY_INPUT_LAYER_TYPE_ID
    assert layers[DIFY_INPUT_LAYER_ID].metadata["origin"] == "workflow_vision_images"
    vision_config = cast(DifyInputLayerConfig, layers[DIFY_INPUT_LAYER_ID].config)
    assert vision_config.images[0].url == "https://files.example/signed/color-card.png"
    assert vision_config.images[0].detail == "high"
    assert layer_names.index(WORKFLOW_USER_PROMPT_LAYER_ID) < layer_names.index(DIFY_INPUT_LAYER_ID)
    assert layer_names.index(DIFY_INPUT_LAYER_ID) < layer_names.index(DIFY_EXECUTION_CONTEXT_LAYER_ID)


def test_request_builder_omits_vision_input_layer_without_images():
    request = AgentBackendRunRequestBuilder().build_for_workflow_node(_run_input())
    assert all(layer.name != DIFY_INPUT_LAYER_ID for layer in request.composition.layers)
