...
def test_invoke_maps_mcp_connection_error_to_503_unavailable() -> None:
    fake_tool = MagicMock()
    fake_app = MagicMock(id="app-1", tenant_id="tenant-1")
    session = MagicMock()
    session.get.return_value = fake_app
    error = ToolInvokeError("Failed to connect to MCP server: timeout")
    error.__cause__ = MCPConnectionError("timeout")

    with (
        patch("services.agent_tool_inner_service.ToolManager.get_agent_tool_runtime", return_value=fake_tool),
        patch("services.agent_tool_inner_service.ToolEngine.generic_invoke", side_effect=error),
    ):
        with pytest.raises(AgentToolInnerServiceError) as exc_info:
            AgentToolInnerService().invoke(_request(), session=session)

    assert exc_info.value.error_code == "agent_tool_mcp_unavailable"
