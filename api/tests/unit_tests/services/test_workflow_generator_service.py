...
class TestWorkflowGeneratorService:
    ...
    @patch("services.workflow_generator_service.build_knowledge_catalogue")
    @patch("services.workflow_generator_service.build_tool_catalogue")
    def test_builds_uncapped_catalogues_and_forwards_searchable_entries(
        self,
        mock_build_tools: MagicMock,
        mock_build_knowledge: MagicMock,
        ...
        dataset = {"id": "dataset-81", "name": "Product Manual", "description": "Current docs."}
        mock_build_tools.return_value = [tool]
        mock_build_knowledge.return_value = [dataset]
        ...
        WorkflowGeneratorService.generate_workflow_graph(
            tenant_id="t-1",
            mode="workflow",
            instruction="Search product docs",
            model_config=_model_config(),
        )

        mock_build_tools.assert_called_once_with("t-1", limit=None)
        mock_build_knowledge.assert_called_once_with("t-1", limit=None, raise_on_error=True)
        kwargs = mock_workflow_generator.generate_workflow_graph.call_args.kwargs
        assert kwargs["tool_catalogue_entries"] == [tool]
        assert kwargs["knowledge_catalogue_entries"] == [dataset]
        assert kwargs["tool_catalogue_text"] == ""
        assert kwargs["knowledge_catalogue_text"] == ""
        assert kwargs["installed_tools"] == {("google", "search")}
        assert kwargs["installed_dataset_ids"] == {"dataset-81"}
        assert kwargs["planner_policy"] == "assume_defaults"
        assert kwargs["context_checkpoint"]["version"] == 4
        assert kwargs["context_checkpoint"]["phase"] == "understanding"
        assert kwargs["context_checkpoint"]["active_turn"]["status"] == "pending_interpretation"
    ...
    def test_forwards_model_instance_and_catalogue_text_to_generator(
        ...
        assert call_kwargs["tool_catalogue_entries"][0]["tool_name"] == "search"
