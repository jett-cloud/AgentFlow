...
class TestToolProviderMCPApi:
    def test_post_does_not_persist_provider_when_handshake_fails(self, app: Flask) -> None:
        api = ToolProviderMCPApi()
        ...
        with (
            ...
            patch(
                "controllers.console.workspace.tool_providers.MCPToolManageService.reconnect_with_url",
                side_effect=ValueError("Failed to connect"),
            ),
            patch("controllers.console.workspace.tool_providers.session_factory.create_session") as create_session,
        ):
            with pytest.raises(ValueError, match="Failed to connect"):
                method(api, "tenant-1", make_account(id="u1"))
