...
def test_workflow_generate_stream_emits_plan_then_result(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _stream(**_kwargs):
        yield ("status", {"stage": "planning", "message": "Planning workflow"})
        yield ("planner_thinking", {"phase": "understanding", "message": "Planning the next action"})
        yield ("turn_interpreted", {"message": "Understood the request"})
        yield ("requirements_resolved", {"count": 1, "labels": ["Goal"]})
        yield ("resource_resolving", {"resource_kind": "dataset"})
        yield ("resource_bound", {"resource_kind": "dataset", "resource_name": "Product docs"})
        yield ("plan", {"title": "Summarizer", "mode": "workflow", "nodes": []})
        yield ("status", {"stage": "building", "message": "Building nodes"})
    ...
    assert frames[7]["stage"] == "building"
    assert frames[6]["title"] == "Summarizer"
    assert frames[8]["mode"] == "workflow"


def test_workflow_generate_stream_passes_through_status_without_pydantic_validation(
    app: Flask, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Status frames bypass plan/result Pydantic models and pass through as JSON."""
    api = generator_module.WorkflowGenerateStreamApi()
    method = unwrap(api.post)
    ...
    assert frames[0]["stage"] == "planning"
    assert frames[0]["message"] == "Planning workflow"


def test_workflow_generate_stream_passes_through_thought_without_pydantic_validation(
    app: Flask, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Thought frames bypass plan/result Pydantic models and pass through as JSON."""
    api = generator_module.WorkflowGenerateStreamApi()
    method = unwrap(api.post)

    def _stream(**_kwargs):
        yield ("status", {"stage": "planning", "message": "Planning workflow"})
        yield ("thought", {"stage": "planner", "delta": '{"nodes":'})
        yield ("result", {"graph": _workflow_graph(), "error": "", "mode": "workflow"})
    ...
    assert frames[1]["event"] == "thought"
    assert frames[1]["stage"] == "planner"
    assert frames[1]["delta"] == '{"nodes":'


def test_workflow_generate_stream_passes_through_operation_without_pydantic_validation(
    app: Flask, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Operation frames bypass plan/result Pydantic models and pass through as JSON."""
    api = generator_module.WorkflowGenerateStreamApi()
    method = unwrap(api.post)
    ...
    def _stream(**_kwargs):
        ...
        yield (
            "operation",
            {"stage": "planning", "action": "plan_ready", "count": 2, "message": "已规划 2 个节点"},
        )
        yield ("result", {"graph": _workflow_graph(), "error": "", "mode": "workflow"})
    ...
    assert frames[1]["event"] == "operation"
    assert frames[1]["action"] == "plan_ready"
    assert frames[1]["count"] == 2
    assert frames[1]["message"] == "已规划 2 个节点"
