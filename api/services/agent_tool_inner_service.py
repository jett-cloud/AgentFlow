...
class AgentToolInnerService:
    ...
    def invoke(self, request: AgentToolInvokeRequest, *, session: Session) -> AgentToolInvokeResponse:
        ...
        try:
            ...
            if isinstance(exc, ToolInvokeError) and _is_mcp_unavailable(exc):
                raise AgentToolInnerServiceError(
                    error_code="agent_tool_mcp_unavailable",
                    description=str(exc),
                    status_code=503,
                ) from exc
            raise AgentToolInnerServiceError(
...
def _is_mcp_unavailable(error: BaseException) -> bool:
    ...
    return isinstance(cause, MCPConnectionError) and not isinstance(cause, MCPAuthError)
