...
from core.mcp.security import select_safe_mcp_icon, validate_remote_mcp_network_target, validate_remote_mcp_url
from core.mcp.types import Tool as MCPTool
from core.tools.entities.api_entities import ToolProviderApiEntity
from core.tools.utils.encryption import ProviderConfigEncrypter
from models.tools import MCPToolProvider

_MCP_ICON_MAX_LEN = 255
...
class ReconnectResult(BaseModel):
    ...
    server_icon: str | None = Field(default=None, description="Safe icon advertised by the remote MCP server")
...
class MCPToolManageService:
    ...
    def create_provider(
        ...
        *,
        ...
        connection_result: ReconnectResult | None = None,
        ...
        mcp_tool = MCPToolProvider(
            ...
            user_id=user_id,
            authed=connection_result.authed if connection_result else False,
        ...
        try:
            ...
            if encrypted_server_url and server_url_hash:
                ...
                if reconnect_result:
                    ...
                    if reconnect_result.server_icon and len(reconnect_result.server_icon) <= _MCP_ICON_MAX_LEN:
                        mcp_provider.icon = reconnect_result.server_icon
    ...
    def _prepare_icon(self, icon: str, icon_type: str, icon_background: str) -> str:
        """Prepare icon data for storage (max 255 chars for DB column)."""
        if icon_type == "emoji":
            result = json.dumps({"content": icon, "background": icon_background})
        else:
            result = icon
        if len(result) > _MCP_ICON_MAX_LEN:
            return ""
        return result
    ...
    def validate_server_url_standalone(
        *,
        ...
        # Validate URL format
        new_server_url = validate_remote_mcp_url(new_server_url)
    ...
    def reconnect_with_url(
        *,
        ...
        server_url = validate_remote_mcp_network_target(server_url)
        ...
        try:
            with MCPClient(
                ...
                return ReconnectResult(
                    ...
                    server_icon=select_safe_mcp_icon(
                        server_url,
                        mcp_client.server_info.icons if mcp_client.server_info else None,
                    ),
                )
        except MCPAuthError:
