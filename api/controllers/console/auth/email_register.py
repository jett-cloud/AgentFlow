...
from ..wraps import decrypt_password_field, email_password_login_enabled, email_register_enabled, setup_required
...
class DevelopmentRegisterPayload(BaseModel):
    email: EmailStr = Field(...)
...
@console_ns.route("/dev-register")  # backward-compatible alias
class DevelopmentRegisterApi(Resource):
    """Self-service register: create an account with its own workspace (no admin invite)."""

    @setup_required
    @email_password_login_enabled
    @email_register_enabled
    @console_ns.expect(console_ns.models[DevelopmentRegisterPayload.__name__])
    @console_ns.response(200, "Success", console_ns.models[SimpleResultResponse.__name__])
    @decrypt_password_field
    def post(self):
        """Register with email/password and establish the normal Dify cookie session."""
        # No-email self-register is available when open registration is on, or when the
        # explicit local/dev flag is set (for deployments that only flip that flag).
        if not (dify_config.ALLOW_REGISTER or dify_config.ALLOW_DEV_NO_EMAIL_REGISTER):
            abort(404)

        args = DevelopmentRegisterPayload.model_validate(console_ns.payload)
        normalized_email = args.email.lower()
        existing_account = AccountService.get_account_by_email_with_case_fallback(
            normalized_email, session=db.session()
        )
        if existing_account:
            raise EmailAlreadyInUseError()
