...
class WorkflowAssistConversationStateTooLarge(BaseHTTPException):
    error_code = "CONVERSATION_STATE_TOO_LARGE"
    description = "Workflow Assist conversation state exceeds the persistence limit."
    code = 413


class WorkflowAssistConversationWriteConflict(BaseHTTPException):
    error_code = "CONVERSATION_WRITE_CONFLICT"
    description = "Workflow Assist conversation was updated concurrently. Please retry."
    code = 409


class WorkflowAssistPlanningSessionConflict(BaseHTTPException):
    error_code = "PLANNING_SESSION_CONFLICT"
    description = "A newer Workflow Assist message was committed first. Please retry from the latest state."
    code = 409


class WorkflowAssistPlanningFailed(BaseHTTPException):
    error_code = "WORKFLOW_ASSIST_PLANNING_FAILED"
    description = "Workflow Assist could not complete planning."
    code = 400

    def __init__(self, planner_code: str, description: str, errors: list[dict] | None = None) -> None:
        self.error_code = planner_code or self.error_code
...
class WorkflowAssistClarificationAlreadyResolved(BaseHTTPException):
    error_code = "CLARIFICATION_ALREADY_RESOLVED"
    description = "The clarification has already been answered."
    code = 409


class WorkflowAssistCandidateBaseStale(BaseHTTPException):
    error_code = "CANDIDATE_BASE_STALE"
    description = "画布已变，候选基于旧版"
    code = 409
