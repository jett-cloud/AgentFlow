...
class ToolPluginStudioSession(TypeBase):
    """Persisted AI tool-plugin studio draft and its last successfully published source."""

    __tablename__ = "tool_plugin_studio_sessions"
    __table_args__ = (
    ...
    phase: Mapped[str] = mapped_column(String(32), nullable=False, default="creating")
    active_tool_name: Mapped[str] = mapped_column(String(255), nullable=False, default="")
    tool_names_json: Mapped[str] = mapped_column(LongText, nullable=False, default="[]")
    files_json: Mapped[str] = mapped_column(LongText, nullable=False, default="{}")
    messages_json: Mapped[str] = mapped_column(LongText, nullable=False, default="[]")
    plugin_status: Mapped[str] = mapped_column(String(32), nullable=False, default="idle")
    plugin_locked_at: Mapped[datetime | None] = mapped_column(sa.DateTime, nullable=True, default=None)
    preview_json: Mapped[str | None] = mapped_column(LongText, nullable=True, default=None)
    ...
    plugin_unique_identifier: Mapped[str | None] = mapped_column(String(512), nullable=True, default=None)
    installation_id: Mapped[str | None] = mapped_column(String(255), nullable=True, default=None)
    revision: Mapped[int] = mapped_column(sa.Integer, nullable=False, default=0, server_default=sa.text("0"))
    published_files_json: Mapped[str] = mapped_column(LongText, nullable=False, default="{}", server_default="{}")
    last_publish_diagnostic_json: Mapped[str | None] = mapped_column(LongText, nullable=True, default=None)
    is_hidden: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, server_default=sa.text("false"), default=False)
    ...
    def tool_names(self) -> list[str]:
        try:
            raw = json.loads(self.tool_names_json or "[]")
    ...
    def files(self) -> dict[str, str]:
        try:
            raw = json.loads(self.files_json or "{}")
        except json.JSONDecodeError:
            return {}
    ...
    @property
    def published_files(self) -> dict[str, str]:
        try:
            raw = json.loads(self.published_files_json or "{}")
        except json.JSONDecodeError:
            return {}
        if not isinstance(raw, dict):
            return {}
    ...
    def messages(self) -> list[dict[str, Any]]:
        try:
            raw = json.loads(self.messages_json or "[]")
        except json.JSONDecodeError:
            return []
        return [_redact_tool_plugin_message_credentials(item) for item in raw if isinstance(item, dict)]
    ...
    def preview(self) -> dict[str, Any] | None:
        if not self.preview_json:
            return None
        try:
            raw = json.loads(self.preview_json)
        except json.JSONDecodeError:
            return None
        return raw if isinstance(raw, dict) else None

    @property
    def last_publish_diagnostic(self) -> dict[str, Any] | None:
        if not self.last_publish_diagnostic_json:
            return None
        try:
            raw = json.loads(self.last_publish_diagnostic_json)
        except json.JSONDecodeError:
            return None
        return raw if isinstance(raw, dict) else None


def _redact_tool_plugin_message_credentials(value: Any) -> Any:
    """Remove legacy runtime credential values before studio history reaches a browser or LLM."""
    if isinstance(value, list):
        return [_redact_tool_plugin_message_credentials(item) for item in value]
    ...
    for key, item in value.items():
        if key == "credentials" and isinstance(item, dict):
            ...
            redacted[key] = _redact_tool_plugin_message_credentials(item)
    return redacted
