...
DIFY_INPUT_LAYER_ID = "vision_input"
type AgentConfigVersionKind = Literal["snapshot", "draft", "build_draft"]


def _append_vision_input_layer(
    ...
    *,
    ...
    layers.append(
        RunLayerSpec(
            name=DIFY_INPUT_LAYER_ID,
            type=DIFY_INPUT_LAYER_TYPE_ID,
            metadata={**metadata, "origin": "workflow_vision_images"},
            config=DifyInputLayerConfig(images=vision_images),
        )
    )
...
class AgentBackendWorkflowNodeRunInput(BaseModel):
    ...
    # Multimodal vision images for models that support vision. Injected as
    # ``dify.input`` after the workflow text user prompt.
    vision_images: list[DifyInputImageConfig] = Field(default_factory=list)
...
class AgentBackendAgentAppRunInput(BaseModel):
    ...
    ask_human_config: DifyAskHumanLayerConfig | None = None
    # Multimodal vision images for models that support vision.
    vision_images: list[DifyInputImageConfig] = Field(default_factory=list)
    # Inject the sandboxed shell layer (dify.shell). Requires the agent backend
    # to be wired with a shellctl entrypoint; see configs AGENT_SHELL_ENABLED.
    include_shell: bool = False
...
class AgentBackendRunRequestBuilder:
    ...
    def build_for_agent_app(self, run_input: AgentBackendAgentAppRunInput) -> CreateRunRequest:
        ...
        _append_vision_input_layer(
            layers,
            vision_images=run_input.vision_images,
        ...
        include_shell = run_input.include_shell
        if include_shell:
        ...
        if run_input.config_layer_config is not None:
            if not include_shell:
                logger.warning(
                    "config_layer_config is set but shell is disabled; "
                    "dify.config requires a shell dependency and will be skipped"
                )
            else:
                layers.append(
                    RunLayerSpec(
        ...
        if run_input.drive_config is not None:
            if not include_shell:
                logger.warning(
                    "drive_config is set but shell is disabled; dify.drive layer requires shell and will be skipped"
                )
            else:
                layers.append(
                    RunLayerSpec(
