...
class WorkflowConfig(BaseSettings):
    ...
    WORKFLOW_ASSIST_MAX_MODEL_CALLS: PositiveInt = Field(
        description="Maximum model invokes (including compaction) in one workflow-assist chat run",
        default=32,
    )
    WORKFLOW_ASSIST_MAX_TOOL_CALLS: PositiveInt = Field(
        description="Maximum tool dispatches in one workflow-assist chat run",
        default=64,
    )
    WORKFLOW_ASSIST_MAX_TOTAL_TOKENS: PositiveInt = Field(
        description="Maximum prompt tokens counted across one workflow-assist chat run",
        default=400_000,
    )
    WORKFLOW_ASSIST_MAX_ELAPSED_TIME: NonNegativeInt = Field(
        description="Maximum wall-clock seconds for one workflow-assist chat run; 0 disables the elapsed-time fuse",
        default=0,
    )
    WORKFLOW_ASSIST_COMPACTOR_INPUT_LIMIT: PositiveInt = Field(
...
class WorkflowAssistRunConfig(BaseSettings):
    """Durable Workflow Assist worker and stale-run recovery limits."""

    WORKFLOW_ASSIST_HEARTBEAT_INTERVAL_SECONDS: PositiveInt = Field(
        description="Seconds between independent Workflow Assist worker heartbeats",
        default=10,
    )
    WORKFLOW_ASSIST_HEARTBEAT_TIMEOUT_SECONDS: PositiveInt = Field(
        description="Seconds before a missing Workflow Assist worker heartbeat is stale",
        default=60,
    )
    WORKFLOW_ASSIST_QUEUE_TIMEOUT_SECONDS: PositiveInt = Field(
        description="Seconds before an unclaimed Workflow Assist run is stale",
        default=120,
    )
    WORKFLOW_ASSIST_REAPER_INTERVAL_SECONDS: PositiveInt = Field(
        description="Seconds between Workflow Assist stale-run scans",
        default=30,
    )
    WORKFLOW_ASSIST_REAPER_BATCH_SIZE: PositiveInt = Field(
        description="Maximum Workflow Assist runs recovered by one stale-run scan",
        default=100,
    )
...
class CeleryScheduleTasksConfig(BaseSettings):
    ...
    ENABLE_WORKFLOW_ASSIST_REAPER_TASK: bool = Field(
        description="Enable scheduled recovery of stale Workflow Assist runs",
        default=True,
    )
...
class LoginConfig(BaseSettings):
    ...
    ALLOW_DEV_NO_EMAIL_REGISTER: bool = Field(
        description="enable no-email self-service /register; also covered when ALLOW_REGISTER is true",
        default=False,
    )
