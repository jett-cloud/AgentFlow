...
def _register_workflow_assist_reaper(beat_schedule: dict[str, CeleryBeatScheduleEntry]) -> None:
    """Register stale-run recovery only when its rollout switch is enabled."""
    if not dify_config.ENABLE_WORKFLOW_ASSIST_REAPER_TASK:
        return
    beat_schedule["workflow_assist_run_reaper"] = {
        "task": "tasks.workflow_assist_run_reaper_task.reap_workflow_assist_runs",
        "schedule": timedelta(seconds=int(dify_config.WORKFLOW_ASSIST_REAPER_INTERVAL_SECONDS)),
    }
...
def init_app(app: DifyApp) -> Celery:
    ...
    imports = [
        ...
        "tasks.workflow_assist_run_task",  # durable Workflow Assist worker
        "tasks.workflow_assist_run_reaper_task",  # stale Workflow Assist run recovery
    ...
    _register_workflow_assist_reaper(beat_schedule)
