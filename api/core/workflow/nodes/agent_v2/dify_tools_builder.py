...
MCP_TOOL_CACHE_TTL = timedelta(minutes=5)
...
class MCPSnapshotRefresher(Protocol):
    def __call__(self, *, tenant_id: str, provider_id: str) -> None: ...
...
def _refresh_stale_mcp_snapshot(*, tenant_id: str, provider_id: str) -> None:
    """Re-run tools/list when the MCP snapshot is older than MCP_TOOL_CACHE_TTL.

    Refresh failure must not block the Agent run; the last stored snapshot is used.
    """
    try:
        session = db.session()
        service = MCPToolManageService(session=session)
        try:
            provider = service.get_provider(server_identifier=provider_id, tenant_id=tenant_id)
        except ValueError:
            provider = service.get_provider(provider_id=provider_id, tenant_id=tenant_id)
        updated_at = provider.updated_at
        if updated_at is not None and datetime.now() - updated_at < MCP_TOOL_CACHE_TTL:
            return
        service.list_provider_tools(tenant_id=tenant_id, provider_id=provider.id)
        session.commit()
    except Exception:
        session.rollback()
        logger.warning(
            "MCP snapshot refresh failed; using cached tools. tenant_id=%s provider_id=%s",
            tenant_id,
            provider_id,
            exc_info=True,
        )
...
class WorkflowAgentDifyToolsBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        mcp_snapshot_refresher: MCPSnapshotRefresher | None = None,
    ) -> None:
        ...
        self._mcp_snapshot_refresher = mcp_snapshot_refresher or _refresh_stale_mcp_snapshot
    ...
    def build_layers(
        ...
        *,
        ...
        self._refresh_mcp_snapshots(tenant_id=tenant_id, enabled_tools=enabled_tools)
    ...
    def _refresh_mcp_snapshots(
        ...
        *,
        ...
        for tool_config in enabled_tools:
            ...
            try:
                self._mcp_snapshot_refresher(tenant_id=tenant_id, provider_id=provider_id)
            except Exception:
                logger.warning(
                    "MCP snapshot refresh failed; using cached tools. tenant_id=%s provider_id=%s",
                    tenant_id,
