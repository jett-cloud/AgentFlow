...
type VisionSupportChecker = Callable[[str, str, str], bool]


def _default_vision_support_checker(tenant_id: str, provider: str, model: str) -> bool:
    return ModelManager().check_model_support_vision(
        tenant_id=tenant_id,
        provider=provider,
        model=model,
...
class WorkflowAgentRuntimeRequestBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        vision_support_checker: VisionSupportChecker | None = None,
    ) -> None:
        ...
        self._vision_support_checker = vision_support_checker or _default_vision_support_checker
    ...
    def _build_workflow_vision_inputs(
        self,
        *,
        tenant_id: str,
        model_provider: str,
        ...
        node_job: WorkflowNodeJobConfig,
        metadata: dict[str, Any],
    ) -> list[DifyInputImageConfig]:
        """Build multimodal vision inputs from workflow IMAGE files.

        Sources: uploaded ``sys.files`` and previous-node IMAGE outputs referenced
        by the node job. Non-image files keep the text download-mapping path in
        the workflow context prompt. When the selected model lacks vision, images
        stay on that text path and a runtime warning is recorded.
        """
        image_files = self._collect_workflow_vision_images(variable_pool, node_job)
        if not image_files:
            return []

        if not self._vision_support_checker(tenant_id, model_provider, model):
            append_runtime_warnings(
                metadata,
                [
                    {
                        "section": "agent_soul.model",
                        "code": "vision_not_supported",
                        "message": (
                            "Workflow images were not injected as vision input "
                            "because the selected model does not support vision."
                        ),
                    }
                ],
            )
        ...
        vision_images: list[DifyInputImageConfig] = []
        skipped_without_url = 0
        for image_file in image_files[:MAX_VISION_IMAGES]:
            url = image_file.generate_url(for_external=True)
        ...
        if len(image_files) > MAX_VISION_IMAGES:
            append_runtime_warnings(
                ...
                [
                    {
                        "section": "workflow.vision_images",
                        "code": "vision_image_limit_exceeded",
                        "message": (
                            f"Only the first {MAX_VISION_IMAGES} workflow image(s) "
                            "were injected as vision input."
                        ),
                    }
                ],
            )
        if skipped_without_url:
            append_runtime_warnings(
                ...
                [
                    {
                        "section": "workflow.vision_images",
                        "code": "vision_image_url_unavailable",
                        "message": (
                            f"{skipped_without_url} workflow image(s) were skipped "
                            "because no external URL could be generated."
                        ),
                    }
                ],
            )
    ...
    def _collect_workflow_vision_images(
        self,
        variable_pool: VariablePoolReader,
        node_job: WorkflowNodeJobConfig,
    ) -> list[File]:
        """Collect IMAGE files from ``sys.files`` and previous-node output refs."""
        collected: list[File] = []
        seen_keys: set[str] = set()

        def _add_if_image(file_obj: Any) -> None:
            if not isinstance(file_obj, File) or file_obj.type != FileType.IMAGE:
                return
            key = self._vision_file_dedupe_key(file_obj)
            if key in seen_keys:
                return
            seen_keys.add(key)
            collected.append(file_obj)

        sys_files = get_system_value(variable_pool, SystemVariableKey.FILES)
        if isinstance(sys_files, list | tuple):
            for item in sys_files:
                _add_if_image(item)

        for ref in self._effective_previous_node_output_refs(node_job):
    ...
    @staticmethod
    def _vision_file_dedupe_key(file_obj: File) -> str:
        if isinstance(file_obj.reference, str) and file_obj.reference.strip():
            return f"ref:{file_obj.reference.strip()}"
        if isinstance(file_obj.remote_url, str) and file_obj.remote_url.strip():
            return f"url:{file_obj.remote_url.strip()}"
        if isinstance(file_obj.id, str) and file_obj.id.strip():
            return f"id:{file_obj.id.strip()}"
        return f"obj:{id(file_obj)}"
