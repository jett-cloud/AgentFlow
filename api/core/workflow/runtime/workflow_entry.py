...
from core.workflow.runtime.variables.variable_prefixes import ENVIRONMENT_VARIABLE_NODE_ID
...
class WorkflowEntry:
    def __init__(
        ...
        # Graphon exposes this context read-only; the Dify entry owns initial binding.
    ...
    def run(self) -> Generator[GraphEngineEvent, None, None]:
        ...
        try:
            # Preserve Dify's response ordering on top of Graphon's raw event stream.
    ...
    def _traced_node_run(node: Node) -> Generator[GraphNodeEventBase, None, None]:
        """
        Run a standalone node with a complete observability lifecycle.

        Finish hooks before yielding the terminal event: callers may stop consuming
        immediately after that event without exhausting the generator.
        """
        layer = ObservabilityLayer()
        layer.initialize(ReadOnlyGraphRuntimeStateWrapper(node.graph_runtime_state), InMemoryChannel())
        node.bind_execution_id(str(uuid4()))

        def _gen() -> Generator[GraphNodeEventBase, None, None]:
