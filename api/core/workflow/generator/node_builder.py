...
from core.workflow.generator.llm_response import LLMJsonClient, StageSchemaError
...
def build_single_node(
    *,
    ...
    """Generate one node's config from intent. Does not write the graph.

    The main-model ``build_node`` tool must call this instead of submitting
    ``config``. ``existing_node`` is prompt context; when its type matches
    ...
    generated = _build_node(
        ...
        plan_json=format_parallel_plan([target_node], [], request.start_inputs),
