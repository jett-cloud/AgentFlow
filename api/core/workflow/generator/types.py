...
# see ``planner.resolve_generation_mode``. No extra LLM call is involved.
...
class WorkflowGenerateErrorCode(StrEnum):
    ...
    PLANNER_ACTION_LIMIT = "PLANNER_ACTION_LIMIT"
    PLANNER_BUDGET_EXHAUSTED = "PLANNER_BUDGET_EXHAUSTED"
    PLANNER_CLARIFICATION_LIMIT = "PLANNER_CLARIFICATION_LIMIT"
    PLANNER_NO_PROGRESS = "PLANNER_NO_PROGRESS"
    PLANNER_CONTEXT_LIMIT = "PLANNER_CONTEXT_LIMIT"
    PLANNER_REQUIREMENT_INVALID = "PLANNER_REQUIREMENT_INVALID"
    PLANNER_POLICY_DENIED = "PLANNER_POLICY_DENIED"
...
class PlannerResourceRequestDict(TypedDict):
    """A planner-declared tool or knowledge-base dependency."""
...
class PlannerResultDict(TypedDict):
    ...
    resource_requests: NotRequired[list[PlannerResourceRequestDict]]
...
class WorkflowGenerateResultDict(TypedDict):
    ...
    mode: NotRequired[str]
    context_checkpoint: NotRequired[dict[str, object]]
