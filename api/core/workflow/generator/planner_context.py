"""Budget and render the observable context for one planner action loop.

The session owns only provider-neutral planner state: the immutable task prompt,
validated actions, deterministic observations, and a compact resume checkpoint.
It never stores hidden reasoning, credentials, or a tenant's complete resource
catalogue. Keeping this responsibility here makes ``planner.iter_plan`` a small
control loop and keeps provider-side prompt-cache prefixes stable until context
pressure requires deterministic compaction.
"""
...
class PlannerContextSession:
            ...
            for history_turn in reversed(request.clarification_history):
                ...
                if isinstance(user_message, str) and user_message.strip():
                    incoming_turn = UserTurn(
                        id=str(
                            history_turn.get("turn_id")
                            or history_turn.get("clarification_id")
                            or "latest-user-turn"
                        ),
                        kind="message",
    ...
    def messages(
        self, *, schema_retry: bool = False, truncation_recovery: bool = False
    ) -> list[PromptMessage]:
