...
from core.workflow.generator.graph.container_nodes import (
    CONTAINER_NODE_TYPES,
    expected_container_start_node_type,
    resolve_container_start_node_id,
)
...
from core.workflow.generator.variables.variable_types import aggregate_type, canonical_value_type, is_array_type
...
def _normalize_iteration_array_type(value: object) -> str:
    value_type = str(value or "")
    return canonical_value_type(value_type) if is_array_type(value_type) else "array"


def _is_official_iteration_array_type(value: object) -> bool:
    return is_array_type(str(value or ""))


def _derive_iteration_output_type(value: object) -> str:
    try:
        return aggregate_type(str(value or ""))
    except ValueError:
        return "array[string]"
...
class GraphPostprocessor:
    ...
    def _postprocess_graph(
        ...
        *,
        ...
        # Persisted if-else cases are authoritative. Canvas branch metadata is
        # derived so stale model-authored labels cannot disagree with Handles.
        cls._derive_if_else_branches(nodes=nodes)
        ...
        cls._derive_iteration_types(nodes=nodes)
        if tool_entries is not None:
            apply_tool_parameters_to_graph(nodes, tool_entries)
    ...
    @classmethod
    def _derive_iteration_types(cls, nodes: list[dict[str, Any]]) -> None:
        for node in nodes:
            data = node.get("data")
            if not isinstance(data, dict) or data.get("type") != BuiltinNodeTypes.ITERATION:
                continue
            input_type = VariableReferences.selector_type(nodes, data.get("iterator_selector"))
            if input_type:
                if _is_official_iteration_array_type(input_type):
                    data["iterator_input_type"] = _normalize_iteration_array_type(input_type)
                else:
                    data["iterator_input_type"] = str(input_type)
            elif not _is_official_iteration_array_type(data.get("iterator_input_type")):
                data["iterator_input_type"] = "array"
            output_type = cls._iteration_terminal_type(nodes, data.get("output_selector"))
            if output_type:
                data["output_type"] = _derive_iteration_output_type(output_type)
            elif not _is_official_iteration_array_type(data.get("output_type")):
                data["output_type"] = "array[string]"

    @classmethod
    def _iteration_terminal_type(cls, nodes: list[dict[str, Any]], selector: object) -> str | None:
        """Prefer a Tool node's declared output type over the generic object fallback."""
        if VariableReferences._is_selector_list(selector) and len(selector) >= 2:
            source = next((node for node in nodes if node.get("id") == selector[0]), None)
            data = source.get("data") if isinstance(source, dict) else None
            if isinstance(data, dict) and data.get("type") == BuiltinNodeTypes.TOOL:
                outputs = data.get("outputs")
                spec = outputs.get(selector[1]) if isinstance(outputs, dict) else None
                if isinstance(spec, dict) and spec.get("type"):
                    return str(spec["type"])
        return VariableReferences.selector_type(nodes, selector)
    ...
    @classmethod
    def _derive_if_else_branches(cls, *, nodes: list[dict[str, Any]]) -> None:
        """Rebuild canvas-only branch metadata from authoritative cases."""
        ...
        for node in nodes:
            ...
            case_ids = [
                case_id
                for case in (data.get("cases") or [])
                if isinstance(case, dict) and isinstance((case_id := case.get("case_id")), str) and case_id
            ]
            multiple_cases = len(case_ids) > 1
            data["_targetBranches"] = [
                {"id": case_id, "name": f"CASE {index + 1}" if multiple_cases else "IF"}
    ...
    def _ensure_container_starts(cls, *, nodes: list[dict[str, Any]], edges: list[dict[str, Any]]) -> None:
        ...
        for container in list(nodes):
            ...
            node_type = data.get("type")
            if node_type not in CONTAINER_NODE_TYPES or not isinstance(node_id, str) or not node_id:
                continue
            is_iteration = node_type == BuiltinNodeTypes.ITERATION
            start_id = resolve_container_start_node_id(node_id=node_id, data=data)
            existing = by_id.get(start_id)
            expected_start_type = expected_container_start_node_type(str(node_type))
            if existing is None:
                start = cls._synthetic_start_node(container_id=node_id, start_id=start_id, is_iteration=is_iteration)
                extras.append(start)
                by_id[start_id] = start
            elif (existing.get("data") or {}).get("type") != expected_start_type:
                # Preserve the collision for structural validation; adding a
                # second node with the same id would hide the real problem.
                continue
            else:
                start = existing
    ...
    @classmethod
    def _container_entry_target(cls, *, body_ids: list[str], start_id: str, edges: list[dict[str, Any]]) -> str | None:
        body = set(body_ids)
    ...
    def _layout_container_subgraphs(cls, *, nodes: list[dict[str, Any]], edges: list[dict[str, Any]]) -> None:
        ...
        containers = [
            ...
            and node.get("id")
            and (node.get("data") or {}).get("type") in CONTAINER_NODE_TYPES
        ]
        ...
        for container in containers:
            ...
            if cls._children_need_auto_layout(children):
                ...
                inner_edges = [
                    edge for edge in edges if edge.get("source") in child_ids and edge.get("target") in child_ids
                ]
                cls._layout_layered_nodes(
                    nodes=children,
                    edges=inner_edges,
    ...
    def _size_container_from_children(cls, container: dict[str, Any], children: list[dict[str, Any]]) -> None:
        ...
        for child in children:
            ...
            max_right = max(max_right, x + cls._node_width(child))
            max_bottom = max(max_bottom, y + cls._node_height(child))
...
def postprocess_graph(
    *,
    ...
    return GraphPostprocessor._postprocess_graph(graph=graph, mode=mode, tool_entries=tool_entries)
