"""Pure, revisioned state transitions for workflow planning conversations.

The model interprets natural language and proposes typed Requirement
Resolutions. This module owns the trust boundary: it validates that each
resolution is supported by the latest user turn and by deterministic resource
searches, then returns a new serializable Planning Session.
"""
    ...
    *,
    ...
    if isinstance(raw_searches, list):
        legacy_search_observations = [
            {**copy.deepcopy(item), "legacy": True}
...
def _remove_resolved_pending(
    session: PlanningSessionState, resolved_keys: frozenset[str]
) -> dict[str, Any]:
    ...
    remaining = [
        ...
        or question.get("requirement_key", question.get("id")) not in resolved_keys
    ]
    if not remaining:
        return {}
...
def reduce_planning_session(
    ...
    normalized_turn = _normalized_evidence(turn.get("message", ""))
    ...
    for resolution in action.resolutions:
        normalized_evidence = _normalized_evidence(resolution["evidence"])
